# Some ANSI escape sequences foro colours and effects
BLACK = '\u001b[30m'
RED = '\u001b[31m'
GREEN = '\u001b[32m'
YELLOW = '\u001b[33m'
BLUE = '\u001b[34m'
MAGENTA = '\u001b[35m'
CYAN = '\u001b[36m'
WHITE = '\u001b[37m'
RESET = '\u001b[0m'

BOLD = '\u001b[1m'
UNDERLINE = '\u001b[4m'
REVERSE = '\u001b[7m'

print(GREEN, "This will be in green")
print("and so is this")


def colour_print(text: str, *effects: str) -> None:
    """
    Print `text` using the ANSI sequences to change colour and formatting
    :param text: The text to print
    :param effects: The effect we want. Zero or more of the constants defined at the start of this module
    """
    effects_string = "".join(effects)   # effects_string joins all the effects passed as an unpacked tuple
    output_string = "{0}{1}{2}".format(effects_string, text, RESET)
    print(output_string)


colour_print("Hello Red", RED, BOLD)
colour_print("Hello Blue reversed underlined", BLUE, REVERSE, UNDERLINE)
print("This should be in th default terminal colour")
colour_print("Hello Yellow", YELLOW)
colour_print("Hello Bold", BOLD)
colour_print("Hello Underline", UNDERLINE)
colour_print("Hello Reverse", REVERSE)
colour_print("Hello Black", BLACK)
